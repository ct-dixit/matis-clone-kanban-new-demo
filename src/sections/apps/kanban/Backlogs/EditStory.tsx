              ...
              <Grid size={12}>
                {story &&
                  ...
                  [...(story?.commentIds || [])].reverse().map((commentId, index) => {
                    ...
                    const profile = commentData ? backlogs?.profiles?.filter((item: KanbanProfile) => item?.id === commentData.profileId)[0] : undefined;
                    return commentData && profile ? <StoryComment key={index} comment={commentData} profile={profile} /> : null;
                  })}
              </Grid>
              <Grid size={12}>
                {story?.id && <AddStoryComment storyId={story.id} />}
              </Grid>
            </Grid>
          </Box>
        </SimpleBar>
