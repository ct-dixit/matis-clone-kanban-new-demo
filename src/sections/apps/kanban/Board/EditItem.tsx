...
import { KanbanItem, KanbanProfile, KanbanUserStory, KanbanColumn, KanbanSubTask } from 'types/kanban';
...
export default function EditItem({ item, profiles, userStory, columns, handleDrawerOpen }: Props) {
  ...
  const [subTasks, setSubTasks] = useState<KanbanSubTask[]>(item.subTasks || []);
                ...
                <Stack spacing={1}>
                  {subTasks.map((subTask) => (
                    <Stack key={subTask.id} direction="row" sx={{ alignItems: 'center', gap: 1 }}>
                      ...
                      />
                      <IconButton size="small" color="error" onClick={() => handleRemoveSubTask(subTask.id)}>
                        <DeleteOutlined />
                      </IconButton>
                    </Stack>
                  ))}
